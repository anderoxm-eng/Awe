...
# ---------- SSH config (written to /tmp to avoid permission issues) ----------
...
# Set root password using openssl + direct shadow manipulation (no PAM needed)
HASHED=$(openssl passwd -6 "$SSH_PASSWORD")
sed -i "s|^root:[^:]*:|root:$HASHED:|" /etc/shadow 2>/dev/null || \
  echo "root:$HASHED:0:0:99999:7:::" >> /etc/shadow
...
supervise_sshd() {
  while true; do
    if ! pgrep -f "sshd -D -f $SSHD_CONFIG" >/dev/null 2>&1; then
      ...
      sleep 2
      pgrep -f "sshd -D -f $SSHD_CONFIG" >/dev/null 2>&1 \
